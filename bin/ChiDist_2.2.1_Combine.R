...
CalcZINB = function(par)
{
  ...
  for (i in 1:dim(data_filtered)[1])
  {
    ...
    gc1 = as.numeric(data_filtered$V13[i])
    gc2 = as.numeric(data_filtered$V14[i])
    if (sum(is.na(expr1))>0 | sum(is.na(expr2))>0 | sum(is.na(cc))>0 | is.na(gc1)>0 | is.na(gc2)>0){
      next()
    }
    aveexpr1 = mean(expr1)
    aveexpr2 = mean(expr2)
...
CalcPValueZINB_Fusion_2.0.0_3 = function(par)
{
  ...
  for (i in 1:dim(data)[1]){
    ...
    gc1 = as.numeric(data$V13[i])
    gc2 = as.numeric(data$V14[i])
    if (sum(is.na(expr1))>0 | sum(is.na(expr2))>0 | sum(is.na(cc))>0 | is.na(gc1)>0 | is.na(gc2)>0){
      Pv[i] = 1
      next()
    }
...
#for (i in 1:dim(data_filtered)[1]){
#    v1 = as.numeric(data_filtered$V13[i])
#    v2 = as.numeric(data_filtered$V14[i])
#    if(is.na(v1) > 0 | is.na(v2) > 0){
#        next()
#    }
#    print(v1)
#    print(v2)
...
for (i in 1:dim(data)[1]) {
  gc1 = as.numeric(data$V13[i])
  gc2 = as.numeric(data$V14[i])
  if (is.na(gc1) > 0 || is.na(gc2) > 0 || max(gc1, gc2) <= 0.05||min(gc1, gc2) >= 0.95) {
    neiflag[i] = 1
  } else{
    neiflag[i] = 0
  }
...
if (length(diff) > 1 && !is.na(var(diff))) {
  thred <- mean(diff) + 1.5 * sqrt(var(diff))
} else {
  thred <- 0
}
print(paste("thred:", thred, sep=""))
for (i in 1:length(gclist)) {
    print("printing inside loop")
    print(paste("i:", i, sep=""))
    print(paste("chimexpr[i]:", chimexpr[i], sep=""))
    print(paste("thred:", thred, sep=""))
...
for (i in 1:dim(data)[1]) {
  ...
  datagcmin[i] = min(as.numeric(data$V14[i]), as.numeric(data$V13[i]))
  datagcmax[i] = max(as.numeric(data$V14[i]), as.numeric(data$V13[i]))
