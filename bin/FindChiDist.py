...
aux_out.write('read_name\tgeneA\tgeneB\tchr1\tpos1\tchr2\tpos2\n')
...
for i in range(int(start), int(last) + 1):
    ...
    for line in allsamfilelines:
        ...
        if thisname == lastname:
            ...
            if len(lines) == 2:
                ...
                info1, info2, info3 = [l.split('\t') for l in lines]
                # strip off version suffix for gene names
                g1 = info1[0].split('.')[0]
                g2 = info2[0].split('.')[0]
                g3 = info3[0].split('.')[0]
                if g1 != g2 or g2 != g3:
                    readlen = len(info1[10])
                    ...
                    geneA = r1[0].split('.')[0]
                    geneB = r2[0].split('.')[0]
                    chrA   = r1[3] if r1[3].startswith('chr') else f"chr{r1[3]}"
                    posA   = r1[4]
                    chrB   = r2[3] if r2[3].startswith('chr') else f"chr{r2[3]}"
                    posB   = r2[4]

                    AddCount(geneA); AddCount(geneB)
                    AddCount2(geneA); AddCount2(geneB)

                    aux_out.write(
                        f"{r1[1]}\t{geneA}\t{geneB}"
                        f"\t{chrA}\t{posA}\t{chrB}\t{posB}\n"
                    )
            lines = [line]
            lastname = thisname
    ...
    # parse chr and pos, skip any malformed entries entirely
        try:
        ...
        # bad chromosome string or missing fields → skip
            continue
    # enforce ordering: smaller chr first, or by position if same chr
        if thischr1 > thischr2 or (thischr1 == thischr2 and pos2 > pos1):
            thischr1, thischr2 = thischr2, thischr1
            pos1,    pos2    = pos2,    pos1
        key = (thischr1, thischr2, pos1, pos2)
        if key not in CandidateVector:
            CandidateVector[key] = []
